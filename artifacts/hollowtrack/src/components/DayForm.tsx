...
export function DayForm({ structure, date, days, onSave, onReset, success }: { structure: Struktur; date: string; days: Tagesdatensatz[]; onSave: (date: string, values: Record<string, Formularwert>, notes: string) => void; onReset: () => void; success: string }) {
  ...
  const resetForm = () => { if (browserDialogs.confirm('Möchtest du alle ungespeicherten Eingaben für diesen Tag wirklich zurücksetzen?')) { const next: Record<string, Formularwert> = {}; getAllTrackers(structure).forEach((item) => { const value = recordValue(record, item); if (valueExists(value)) next[item.id] = value; }); setValues(next); setNotes(record?.notizen || ''); onReset(); } };
