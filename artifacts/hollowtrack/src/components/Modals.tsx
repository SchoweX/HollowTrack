...
export function Modal({ modal, structure, name, parentId, inputType, dataType, erfassungsart, gewichtAktiv, trainingsgewicht, setName, setParentId, setInputType, setDataType, setErfassungsart, setGewichtAktiv, setTrainingsgewicht, onClose, onSubmit }: { modal: Exclude<ModalState, null>; structure: Struktur; name: string; parentId: string; inputType: Eingabetyp; dataType: TrackerDatentyp; erfassungsart: 'einzelwert' | 'saetze'; gewichtAktiv: boolean; trainingsgewicht: string; setName: (value: string) => void; setParentId: (value: string) => void; setInputType: (value: Eingabetyp) => void; setDataType: (value: TrackerDatentyp) => void; setErfassungsart: (value: 'einzelwert' | 'saetze') => void; setGewichtAktiv: (value: boolean) => void; setTrainingsgewicht: (value: string) => void; onClose: () => void; onSubmit: () => void }) {
...
) : null}<div className="modal__actions"><button className="button button--quiet" type="button" onClick={onClose}>Abbrechen</button><button className="button button--primary" type="submit" disabled={(!isMove && !name.trim()) || (needsParent && !parentId)}><Check size={15} />Speichern</button></div></form></section></div>;
}
